...
export function InviteStudentButton() {
  ...
  const handleInviteUsers = async () => {
    ...
    try {
      // Send invitations using Clerk's organization.inviteMember
      const results = await Promise.allSettled(
        allEmails.map((email) =>
          organization.inviteMember({
            emailAddress: email,
            role: 'org:student', // Use the student role (Clerk role name)
          })
        )
      );
      ...
      if (failures.length === 0) {
        setSuccessMessage(
          `Successfully invited ${allEmails.length} user${allEmails.length > 1 ? 's' : ''}!`
        );
        resetModalState();
        setTimeout(() => {
          setOpen(false);
        ...
        const successCount = allEmails.length - failureCount;
        setValidationErrors([
          `${successCount} invitation${successCount !== 1 ? 's' : ''} sent successfully. ${failureCount} failed.`,
        ]);
      }
    } catch (error) {
