...
import { Play, Copy, Check, ChevronDown, ChevronRight, Brain, Wrench, MessageSquare, Trash2, Settings, Save } from 'lucide-react';
...
const AVAILABLE_MODELS = [
  ...
  { value: 'claude-3-7-sonnet-20250219', label: 'Claude 3.7 Sonnet (Anthropic)' },
...
async function sendDebugMessage(params: {
  bot_id: string;
  message: string;
...
export default function AgentConsolePage() {
  ...
  const [toolParamOverrides, setToolParamOverrides] = useState<Record<string, any>>({});
  ...
  const loadBots = async (orgId: string) => {
    try {
      ...
      console.log('Loaded bots for org:', orgId, botsData);
      ...
      if (sortedBots.length > 0) {
        ...
        console.log('First bot:', firstBot);
        console.log('First bot system_prompt:', firstBot.system_prompt);
        if (firstBot.system_prompt) {
          console.log('Setting system prompt to:', firstBot.system_prompt);
          setSystemPrompt(firstBot.system_prompt);
        } else {
          console.log('No system_prompt found on first bot');
        }
    ...
    try {
      ...
      await sendDebugMessageStream(params, {
        ...
        onThinkingDelta: (data) => {
          ...
          if (currentThinkingIteration !== null) {
            for (let i = streamingTimeline.length - 1; i >= 0; i--) {
              if (streamingTimeline[i].type === 'thinking' &&
          ...
          if (thinkingIndex !== -1) {
            ...
            streamingTimeline[thinkingIndex].content = (streamingTimeline[thinkingIndex].content || '') + data.delta;
            streamingTimeline[thinkingIndex].tokens_used = Math.ceil((streamingTimeline[thinkingIndex].content?.length || 0) / 4);
          } else {
            // Fallback: create new entry if we can't find it (shouldn't happen)
            console.warn('Could not find thinking block for iteration:', currentThinkingIteration);
            streamingTimeline.push({
              type: 'thinking',
        ...
        onToolStart: (data) => {
          ...
          streamingToolData[data.tool_id] = { tool: data.tool, order: data.order, iteration: data.iteration };
          const timelineIndex = streamingTimeline.length;
          streamingTimeline.push({
            type: 'tool_call',
            iteration: data.iteration,
        ...
        onToolExecuting: (data) => {
          ...
          let toolIndex = -1;
          for (let i = streamingTimeline.length - 1; i >= 0; i--) {
            if (streamingTimeline[i].type === 'tool_call' &&
  ...
  const handleClearConversation = () => {
    const confirmed = window.confirm('Clear conversation history? This will reset the debug session.');
    if (confirmed) {
      setConversationHistory([]);
  ...
  const toggleResponseCard = (responseIndex: number) => {
    setConversationResponses(prev =>
      prev.map((r, i) => i === responseIndex ? { ...r, expanded: !r.expanded } : r)
    );
  };
  ...
  const handleSaveSystemPrompt = async () => {
    ...
    try {
      const response = await fetch(`/api/mentor-bots/${selectedBot}/system-prompt`, {
        method: 'PATCH',
        headers: {
  ...
  const renderTimeline = (timeline: TimelineEvent[] | undefined, responseIndex?: number) => {
    ...
    return timeline
      ...
      .map((event, index) => {
        ...
        const eventKey = responseIndex !== undefined ? `${responseIndex}-${index}` : `current-${index}`;
        ...
        if (event.type === 'thinking') {
          return (
            ...
            >
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm font-mono text-purple-400">
                    Thinking{event.iteration !== undefined ? ` (Round ${event.iteration})` : ''}
                  </span>
                </div>
              ...
              {expandedTimeline.has(index) && (
                <div className="p-3 border-t" style={{ borderColor: '#7c3aed' }}>
          ...
          return (
            ...
            >
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  {event.iteration !== undefined && (
                    <span className="text-xs text-gray-500">Round {event.iteration}</span>
                  )}
                </div>
                <span className="text-xs text-gray-500">
                  {event.tokens_used || 0} tokens
              ...
              {expandedTimeline.has(index) && (
                <div className="p-3 border-t space-y-2" style={{ borderColor: '#10b981' }}>
                  {event.query && (
                    <div>
                      <div className="text-xs font-medium text-gray-400 mb-1">Query:</div>
                  ...
                  {event.results && (
                    <div>
                      <div className="text-xs font-medium text-gray-400 mb-1">Results:</div>
                      <div className="text-sm font-mono text-gray-300 whitespace-pre-wrap">
                        {event.results}
                      </div>
                    </div>
          ...
          return (
            ...
            >
              <div className="p-3 flex items-center space-x-2 border-b" style={{ borderColor: '#3b82f6' }}>
                <MessageSquare className="w-4 h-4 text-blue-400" />
                <span className="text-sm font-mono text-blue-400">Final Answer</span>
  ...
  return (
    <div className="h-screen flex flex-col" style={{ backgroundColor: '#111318', color: '#e0e0e0' }}>
      {/* Top Bar - User Controls */}
      <div className="border-b px-4 py-2 flex items-center justify-between" style={{ borderColor: '#2a2d35', backgroundColor: '#0a0b0e' }}>
        <span className="text-xs font-semibold text-emerald-400">AGENT CONSOLE</span>
        <UserButton afterSignOutUrl="/sign-in" />
      </div>

      {/* Header */}
      <header className="border-b px-6 py-3 flex items-center justify-between" style={{ borderColor: '#2a2d35' }}>
        <div className="flex items-center space-x-4">
          <SuperAdminOrganizationPicker
            onOrganizationChange={setSelectedOrgId}
            className="px-3 py-1.5 rounded border text-sm"
            style={{ backgroundColor: '#1a1d23', borderColor: '#2a2d35', color: '#e0e0e0' }}
          />
          ...
          >
            {bots.length === 0 ? (
              <option value="">
                {selectedOrgId ? 'No bots in this organization' : 'Select an organization first'}
              </option>
            ) : (
              bots.map((bot) => (
          ...
          <div className="flex items-center space-x-2">
            ...
            {selectedBot && selectedModel && bots.find(bot => bot.id === selectedBot)?.model_name !== selectedModel && (
              <button
      ...
      {/* Error Display */}
      {error && (
        <div className="mx-6 mt-4 p-3 rounded border" style={{ backgroundColor: '#991b1b', borderColor: '#dc2626', color: '#fca5a5' }}>
          {error}
        </div>
      )}
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-1/3 border-r flex flex-col" style={{ borderColor: '#2a2d35' }}>
          <div className="p-4 border-b" style={{ borderColor: '#2a2d35' }}>
          ...
          <div className="flex-1 p-4 flex flex-col overflow-hidden">
            ...
            {/* Save Changes Bar */}
            {promptModified && (
              <div className="p-3 rounded flex items-center justify-between" style={{ backgroundColor: '#4ade80', color: '#111318' }}>
            ...
            {/* Save Error Display */}
            {saveError && (
              <div className="mt-2 p-2 rounded border text-sm" style={{ backgroundColor: '#991b1b', borderColor: '#dc2626', color: '#fca5a5' }}>
                {saveError}
              </div>
            )}
          </div>
        ...
        <div className="w-1/3 border-r flex flex-col" style={{ borderColor: '#2a2d35' }}>
          ...
          {availableTools.length > 0 && (
            <div className="border-b" style={{ borderColor: '#2a2d35' }}>
              <div className="p-4 border-b" style={{ borderColor: '#2a2d35' }}>
                <span className="font-medium text-sm">Tools</span>
              </div>
              <div className="p-4 max-h-64 overflow-y-auto">
                {availableTools.map((tool) => (
                  <div key={tool.name} className="mb-4 last:mb-0">
                    <div className="flex items-start space-x-2 mb-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="space-y-2">
                          {Object.entries(tool.input_schema.properties).map(([paramName, paramDef]) => {
                            const isRequired = tool.input_schema.required?.includes(paramName);
                            ...
                            if (paramName === 'num_results' && paramDef.type === 'integer') {
                              const currentValue = toolParamOverrides[tool.name]?.num_results ?? defaultValue ?? 7;
                                ...
                                <div key={paramName} className="space-y-1">
                                  <div className="flex items-center justify-between">
                                    <label className="text-xs text-gray-400">
                                      {paramName}
                                      {!isRequired && <span className="text-gray-600 ml-1">(optional)</span>}
                                    </label>
                                    <span className="text-xs text-emerald-400 font-mono">{currentValue}</span>
                                  </div>
                                  <input
                                    type="range"
                                    min="1"
                                  ...
                                  {paramDef.description && (
                                    <div className="text-xs text-gray-500">{paramDef.description}</div>
                                  )}
                                </div>
                              );
                            }
        ...
        <div className="w-1/3 flex flex-col">
          <div className="p-4 border-b" style={{ borderColor: '#2a2d35' }}>
            <span className="font-medium text-sm">Responses{conversationResponses.length > 0 ? ` (${conversationResponses.length})` : ''}</span>
          </div>
          <div className="flex-1 p-4 flex flex-col overflow-hidden">
            {loading && streamingStatus && (
              <div className="mb-3 p-2 rounded" style={{ backgroundColor: '#1a1d23', borderLeft: '3px solid #4ade80' }}>
                <div className="text-xs font-medium text-emerald-400">{streamingStatus}</div>
              </div>
            )}
            <div className="flex-1 overflow-y-auto mb-4 space-y-3">
              {loading && !debugResponse ? (
                ...
                <>
                  ...
                  {loading && debugResponse?.debug_info?.timeline && debugResponse.debug_info.timeline.length > 0 && (
                    <div className="space-y-3">
                      ...
                      {conversationResponses.length > 0 && (
                        <div className="border-t pt-3" style={{ borderColor: '#2a2d35' }}>
                          <div className="text-xs text-gray-500 px-2 mb-3">Previous Responses</div>
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {conversationResponses.length > 0 ? (
                    conversationResponses.map((response, responseIndex) => {
                      const truncatedMessage = response.userMessage.length > 80
                        ? response.userMessage.substring(0, 80) + '...'
                        : response.userMessage;
                      const tokens = response.debugResponse.debug_info?.total_tokens.output || 0;
                      const responseTime = response.debugResponse.debug_info?.response_time_ms || 0;

                      return (
                        <div
                          key={response.timestamp}
                          className="border rounded"
                          style={{ borderColor: '#2a2d35', backgroundColor: '#1a1d23' }}
                        >
                          <button
                          ...
                          {response.expanded && response.debugResponse.debug_info?.timeline && (
                            <div className="p-3 border-t space-y-3" style={{ borderColor: '#2a2d35' }}>
                              {renderTimeline(response.debugResponse.debug_info.timeline, responseIndex)}
                            </div>
                          )}
                        </div>
                      );
            ...
            <div className="flex items-center justify-end pt-4 border-t" style={{ borderColor: '#2a2d35' }}>
              <div className="flex items-center space-x-4">
                <span className="text-xs text-gray-500">{responseTokens} tokens</span>
                <button
                  onClick={handleCopy}
