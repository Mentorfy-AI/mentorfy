...
export async function GET(
  ...
  try {
    ...
    // Super admins bypass RLS to access any org's agents
    const superAdmin = await isSuperAdmin()
    const supabase = superAdmin
      ? createServiceClient()
      : await createClerkSupabaseClient()
    const { agentId } = params

    // Get agent details (RLS filters by org automatically for non-super-admins)
...
export async function PATCH(
  ...
  try {
    ...
    // Super admins bypass RLS to update any org's agents
    const superAdmin = await isSuperAdmin()
    const supabase = superAdmin
      ? createServiceClient()
      : await createClerkSupabaseClient()
...
export async function DELETE(
  ...
  try {
    ...
    // Super admins bypass RLS to delete any org's agents
    const superAdmin = await isSuperAdmin()
    const supabase = superAdmin
      ? createServiceClient()
      : await createClerkSupabaseClient()
