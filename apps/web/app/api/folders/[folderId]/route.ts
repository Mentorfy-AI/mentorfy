 ...
 * Delete folder and all nested content (documents, subfolders)
 *
 * This endpoint:
 * 1. Recursively collects all document IDs from the folder and subfolders
 * 2. Calls deleteDocuments() to cascade deletion through knowledge graph cleanup
 * 3. Deletes the folder structure
 *
 * This ensures that all documents go through proper cleanup (Graphiti episodes, storage, etc.)
 * rather than just relying on database CASCADE which misses the knowledge graph.
 */
export async function DELETE(
  ...
  try {
    ...
    // Collect all document IDs recursively from folder and subfolders
    const documentIds = await collectAllDocumentIds(supabase, folderId)

    // Delete all documents via consolidated deletion function
    // This ensures knowledge graph cleanup for each document
    let deletedDocCount = 0
    if (documentIds.length > 0) {
      const { deleteDocuments } = await import('@/lib/document-deletion')
      const result = await deleteDocuments(documentIds, orgId)

      if (!result.success) {
        console.error('Error deleting documents in folder:', result.error)
        return NextResponse.json(
          { success: false, error: `Failed to delete documents in folder: ${result.error}` },
          { status: 500 }
        )
      }

      deletedDocCount = result.deleted_count || 0
    }

    // Delete folder (RLS enforces org filtering)
    // Subfolders will be deleted by CASCADE constraint
...
/**
 * Recursively collect all document IDs from a folder and its subfolders
 */
async function collectAllDocumentIds(supabase: any, folderId: string): Promise<string[]> {
  ...
  while (folderQueue.length > 0) {
    ...
    if (docsError) {
      console.error(`Error fetching documents for folder ${currentFolderId}:`, docsError)
    } else if (documents) {
      allDocIds.push(...documents.map((d: any) => d.id))
    }
    ...
    if (childrenError) {
      console.error(`Error fetching child folders for ${currentFolderId}:`, childrenError)
    } else if (children) {
      folderQueue.push(...children.map((c: any) => c.id))
    }
  }
