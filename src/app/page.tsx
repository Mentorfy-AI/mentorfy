...
import { MentorfyWatermark } from '@/components/flow/shared/MentorfyWatermark'
...
// Mentorfy Avatar Component
function MentorfyAvatar({ size = 88 }: { size?: number }) {
  return (
    <div
      style={{
        width: `${size}px`,
        ...
        boxShadow: '0 0 6px rgba(0, 0, 0, 0.4), 0 0 16px rgba(0, 0, 0, 0.25), 0 0 32px rgba(0, 0, 0, 0.1)',
      }}
    >
      <svg
        width={size * 0.5}
  ...
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
      <span style={{
        ...
        fontWeight: '600',
        color: '#111',
        fontFamily: TYPOGRAPHY.FONT_SERIF,
      }}>
        Mentorfy AI
      </span>
...
export default function Home() {
  ...
  const handleSelectRevenue = (revenue: string) => {
  ...
  const isContactValid = formData.name.length >= 2 &&
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: BACKGROUND_COLOR }}>
      <AnimatePresence mode="wait">
        {step === 0 && <LandingStep key="landing" onStart={handleNext} />}
        {step === 1 && (
          <SelectionStep
            key="role"
        ...
        {step === 4 && <ConfirmationStep key="confirmation" name={formData.name} />}
      </AnimatePresence>
    </div>
  )
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ color: ACCENT_COLOR }}>AI Education Product</span>
        {' '}That Converts<br />
        <span style={{ textDecoration: 'underline', textDecorationThickness: '2px', textUnderlineOffset: '4px' }}>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.98 }}
        style={{
          background: 'linear-gradient(to bottom, #12c48a 0%, #10B981 50%, #0ea572 100%)',
          color: '#FFFFFF',
          padding: '18px 48px',
          borderRadius: '14px',
          fontSize: '17px',
...
function SelectionStep({ question, options, selected, onSelect, onBack, stepLabel }: SelectionStepProps) {
  return (
    <motion.div
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {options.map((option) => (
          <motion.button
            ...
            whileTap={{ scale: 0.99 }}
            style={{
              backgroundColor: selected === option ? 'rgba(16, 185, 129, 0.08)' : '#FFFFFF',
              border: selected === option ? '2px solid rgba(16, 185, 129, 0.5)' : '1px solid #EBEBEB',
              borderRadius: '12px',
              padding: selected === option ? '15px 19px' : '16px 20px',
              fontFamily: TYPOGRAPHY.FONT_SERIF,
              fontSize: '17px',
              color: '#111',
              textAlign: 'left',
...
function ContactStep({ formData, focusedField, setFocusedField, onChange, onSubmit, onBack, isValid }: ContactStepProps) {
  const fields = [
    { key: 'name', placeholder: 'Full Name', type: 'text', autoComplete: 'name' },
    { key: 'email', placeholder: 'Email', type: 'email', autoComplete: 'email' },
    { key: 'phone', placeholder: 'Phone', type: 'tel', autoComplete: 'tel' },
  ]
...
function ConfirmationStep({ name }: { name: string }) {
