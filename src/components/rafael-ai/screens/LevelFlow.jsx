...
function MultipleChoiceStep({ step, onAnswer, onBack, currentStep, totalSteps }) {
  ...
  return (
    <motion.div
      ...
      animate="animate"
      exit="exit"
      style={{ backgroundColor: '#FAFAFA', minHeight: '100vh', position: 'relative', display: 'flex', flexDirection: 'column' }}
    >
      {/* Back Button */}
...
function LongAnswerStep({ step, onAnswer, onBack, currentStep, totalSteps }) {
    ...
    >
      ...
      <div style={{
        ...
        <div style={{
          ...
          border: isFocused || value ? '1px solid #000000' : '1px solid #E0E0E0',
          borderRadius: '12px',
          padding: '16px',
          minHeight: '140px',
          marginBottom: '16px',
...
function ContactInfoStep({ step, onAnswer, onBack, currentStep, totalSteps }) {
          ...
          {step.fields.map((field) => (
            <input
              ...
              style={{
                ...
                padding: '16px 20px',
                backgroundColor: '#FFFFFF',
                border: focusedField === field.key || values[field.key] ? '1px solid #000000' : '1px solid #E0E0E0',
                borderRadius: '12px',
                fontSize: '16px',
                fontFamily: "'Geist', -apple-system, sans-serif",
                color: '#111',
...
function ThinkingStep({ step, onComplete }) {
  return (
    <motion.div
      variants={pageVariants}
      initial="initial"
...
function AIMomentStep({ step, state, onContinue, currentStep, totalSteps }) {
...
function VideoStep({ step, onContinue, currentStep, totalSteps }) {
  const video = mentor.videos[step.videoKey]

  // Streaming animation state for intro text
  const [streamedText, setStreamedText] = useState('')
  const [streamingComplete, setStreamingComplete] = useState(!step.introText)
    ...
    >
      ...
      <div style={{
          ...
          <div style={{
            ...
            {displayParagraphs.map((paragraph, i) => (
              <p key={i} style={{
                ...
                {i === displayParagraphs.length - 1 && !streamingComplete && (
                  <span style={{
                    display: 'inline-block',
                    width: '2px',
                    height: '1em',
        ...
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <VideoEmbed url={video?.url} maxWidth="560px" />
        </div>
